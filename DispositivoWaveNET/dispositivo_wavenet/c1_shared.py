...
BASE_FREQ = 300			   # Frecuencia para byte 0
SILENCE_FREQ = 150
FREQ_STEP = 80
...
def escuchar_y_retornar_trama(timeout, ping_timeout):
	"""
	...
	while since_byte < 10:
		...
		if (heard_ping and time.time() - start_time > timeout):
			...
			break
		if (not heard_ping and time.time() - start_time > ping_timeout):
			logging.info("Timeout alcanzado. Terminando escucha.")
			break
			
		duracion_muestra = int(SAMPLE_RATE * (BYTE_DURATION-0.08))

		audio = sd.rec(duracion_muestra, samplerate=SAMPLE_RATE, channels=1, dtype='float64', blocking=True)
...
def transmite_freq(freq, duration=BYTE_DURATION):
	...
	sd.play(tono, samplerate=SAMPLE_RATE, blocking=True)
