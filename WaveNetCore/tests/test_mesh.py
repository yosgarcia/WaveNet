...
def test_all_nodes_can_join_network(setup_mesh_2):
	...
	for node in nodes:
		...
		assert node.node.info.ID in hub.nodes

def test_hub_can_ping_node(setup_mesh_3):
	hub, nodes = setup_mesh_3
	for node in nodes:
		assert hub.ping(node.node.info.ID)
...
def test_node_can_ping_node(setup_mesh_3):
	...
	assert nodes[0].ping(nodes[1].node.info.ID)
	assert nodes[1].ping(nodes[0].node.info.ID)
...
def test_recv_data_from_any(setup_mesh_3):
	...
	def recv_thread():
		result["from_id"], result["data"] = receiver.recv_data(ID=None)

	t = Thread(target=recv_thread)
	t.start()
	time.sleep(0.5)
	sender.send_data(receiver.node.info.ID, msg)
	t.join(timeout=2)
...
def test_recv_data_from_specific_id(setup_mesh_3):
	...
	def recv_thread():
		result["from_id"], result["data"] = receiver.recv_data(ID=target_sender.node.info.ID)

	t = Thread(target=recv_thread)
	t.start()
	time.sleep(0.1)

	other_sender.send_data(receiver.node.info.ID, "Shalam alakum")
	time.sleep(0.1)

	target_sender.send_data(receiver.node.info.ID, msg)
	t.join(timeout=2)

	assert result.get("data") == msg
	assert result.get("from_id") == target_sender.node.info.ID
